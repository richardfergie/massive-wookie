...
acidStateCrud :: (Monad m, MonadIO m, MonadError Crud.CRUDError m) => AcidState World -> Program Crud.Crud a -> m a
acidStateCrud acid p = case Op.view p of
  Return x -> return x
  Crud.GetGroup x :>>= ps -> (liftIO $ putStrLn "Get Group") >> query' acid (GetGroup x) >>= maybe (throwError Crud.NotFound) (acidStateCrud acid . ps . Just) --acidStateCrud acid . ps
  Crud.DeleteGroup x :>>= ps -> do
    liftIO $ putStrLn "Delete Group"
    n <- update' acid (DeleteGroup x)
    acidStateCrud acid $ ps n
  Crud.SetGroup k v :>>= ps -> (liftIO $ putStrLn "Insert Group") >> update' acid (UpdateGroup k v) >>= \res -> either (throwError) (acidStateCrud acid . ps) res
  Crud.CreateGroup v :>>= ps -> (liftIO $ putStrLn "Create Group") >> update' acid (InsertGroup v) >>= \res -> either throwError (acidStateCrud acid . ps) res
  Crud.GetGroupMember x :>>= ps -> (liftIO $ putStrLn "Get GroupMember") >> query' acid (GetGroupMember x) >>= maybe (throwError Crud.NotFound) (acidStateCrud acid . ps . Just)
